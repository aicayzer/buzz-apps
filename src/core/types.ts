...
export interface Config {
  relayUrl: string; identities: Record<string, { botKey: string; authTag?: string }>; setupToken?: string;
...
export interface SendOptions { dedupKey?: string; root?: string; broadcast?: boolean; mentions?: string[]; edit?: string }
export interface BuzzTransport {
  pubkey: string;
  send(channel: string, content: string, options?: SendOptions): Promise<string>;
  dm(pubkey: string, content: string, options?: Pick<SendOptions, 'dedupKey'>): Promise<string>;
...
export interface AppContext { config: Config; store: StoreApi; buzz: BuzzTransport; account?(pubkey: string): Promise<Account | undefined>; link?(purpose: 'signin' | 'settings' | 'open' | 'reminders' | 'issue-edit', message: Message, data?: Record<string, unknown>): Promise<string>; log: { info(obj: unknown, msg?: string): void; error(obj: unknown, msg?: string): void; warn(obj: unknown, msg?: string): void } }
